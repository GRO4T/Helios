...
#include "Helios/Core/DisplayManager.hpp"
#include "Helios/Core/Utils.hpp"
#include "Helios/Core/renderer/MasterRenderer.hpp"

namespace helios {

/**
 * Main class of the game engine. Handles rendering and keeps track of the
 * camera, entities and light sources.
 */
class GameEngine {
 public:
  GameEngine()
   ...
   * @param model_file_path         Path to the *.obj containing entity's 3D
   * model.
   */
...
am mesh                    Light's mesh. TODO: Make more descriptive.
   */
  void registerPointLight(const Transform& transform, MeshPtr mesh);

 private:
  static const uint32_t DISPLAY_WIDTH =
      1440;  //< Application window width in pixels.
  static const uint32_t DISPLAY_HEIGHT =
      900;                             //< Application window height in pixels.
