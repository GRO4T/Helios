...
Model::Model(const std::string& path) {
  ...
  load(path);
  b.end();
  std::cout << "loaded in: " << b.getElapsedTime<std::chrono::milliseconds>()
...
MaterializedMeshSharedPtr Model::processMesh(aiMesh* mesh,
                                             const aiScene* scene) {
  ...
  return std::make_shared<MaterializedMesh>(
      std::move(std::make_unique<IndexedMesh>(vertices, indices)), material);
}
